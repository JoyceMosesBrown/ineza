...
import { Moon, Utensils, Droplets, Heart, Brain, MessageCircle, CheckCircle } from 'lucide-react';
...
export default function Tracker() {
  ...
  const Q = ({ Icon, question, children }: {
    ...
    <div style={{ padding: '20px 0', borderBottom: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <Icon size={20} strokeWidth={1.8} color="var(--green-600)" />
      ...
      {[1, 2, 3, 4, 5].map(v => (
        <button key={v} onClick={() => setBond(v)}
          ...
          <Heart
            size={20}
            strokeWidth={1.8}
            color={bond >= v ? 'var(--green-700)' : 'var(--gray-300)'}
            fill={bond >= v ? 'var(--green-100)' : 'none'}
          />
        </button>
      ))}
    </div>
  ...
  return (
    <div className="page" style={{ maxWidth: 680 }}>
      ...
      <div className="card card-pad">
        ...
        <Q Icon={Utensils} question={t('ateQ')}>
        ...
        <Q Icon={Droplets} question={t('criedQ')}>
          <div className="option-group">
            {[true, false].map(v => (
        ...
        <Q Icon={Heart} question={t('bondQ')}>
          <BondRating />
        </Q>

        <Q Icon={Brain} question={t('anxQ')}>
          <div className="option-group">
        ...
        <Q Icon={MessageCircle} question={t('socialQ')}>
          <div className="option-group">
            {[true, false].map(v => (
        ...
        <div style={{ paddingTop: 24 }}>
          {saved ? (
            <div style={{ background: 'var(--green-100)', color: 'var(--green-700)', borderRadius: 10, padding: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontWeight: 700, fontSize: 15 }}>
